export function getSaasUsersPhpCode(): string {
  return `<?php
/**
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SERVER['CONTENT_TYPE']) && strpos($_SERVER['CONTENT_TYPE'], 'application/json') !== false) {
    ...
    if ($action === 'update_plan') {
        ...
        $newFee = ($newPlan === 'enterprise') ? 199.00 : (($newPlan === 'pro') ? 29.00 : 0.00);
        $newQuota = ($newPlan === 'enterprise') ? 10000000 : (($newPlan === 'pro') ? 1000000 : 100000);
                ...
                <div class="bg-zinc-900/90 border-2 border-orange-500/50 rounded-2xl p-6 space-y-4 shadow-xl shadow-orange-500/10 flex flex-col justify-between relative">
                    ...
                    <div class="space-y-2">
                        ...
                        <ul class="text-xs text-zinc-400 space-y-1.5 pt-2 border-t border-white/5">
                            ...
                            <li class="flex items-center gap-2"><span class="text-orange-400">&bull;</span> Real-time cost metering</li>
                            <li class="flex items-center gap-2"><span class="text-orange-400">&bull;</span> Multi-tenant simulation</li>
                        </ul>
                    </div>
...
createApp({
    setup() {
        ...
        const navigate = (path) => {
            // Clean extension-free URL routing
            if (path === '/saas-users') window.location.href = '/saas-users';
            else if (path === '/saas-reports') window.location.href = '/saas-reports';
            else if (path === '/register') window.location.href = '/register';
